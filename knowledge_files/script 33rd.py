    ...
    for d in duration_list:
        ...
        if d != 1:
            ...
            for i in range(timespan.shape[1]):
                exp_sum += np.exp(-i/5) * timespan.iloc[:,i] #为什么不直接用累加
...
#编码的类别有：商店，商店类型，商店簇，商品大类，商品小类
cat_features = ['store_nbr','type','cluster','family','class']
num_features = [i for i in X_train.columns if i not in cat_features] #写法***

for col in cat_features:
    le = LabelEncoder()
    # 训练验证测试共同编码
    le.fit(pd.concat([X_train[col].drop_duplicates(), X_val[col].drop_duplicates(), X_test[col].drop_duplicates()]))#drop_duplicates去重
...
cat_count_features = []
for col in cat_features:
    d = pd.concat([X_train[col],X_val[col],X_test[col]]).value_counts().to_dict() #to_dict()
    ...
    #dict['key']只能获取存在的值，如果不存在则触发KeyError，而dict.get(key, default=None)则如果不存在则返回一个默认值，如果设置了则是设置的，此处0为默认，否则就是None
...
for i in range(16):
    ...
    dtrain = lgb.Dataset(X_train, label=y_train[:, i],#这是一天一天地预测的？？？
